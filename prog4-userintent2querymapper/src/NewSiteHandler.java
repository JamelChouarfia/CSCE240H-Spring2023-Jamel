...
public class NewSiteHandler {
	...
	public static void printQuery(ArrayList<String> potentialFiles, String query) {
		try {
			if (potentialFiles.isEmpty()) {
				System.out.println("Sorry, I can not connect to the internet");
				return;
			}
			
			String[][] answers = new String[150][100];
			int[] answerStrength = new int[150];
			int stringCounter = 0;
			int answerCounter = 0;
			int fileCounter = 0;
			int results = 0;
			
			while (potentialFiles.size() > fileCounter) {
				String temp = potentialFiles.get(fileCounter);
				answerStrength[results] = LockMatch.lock_match(temp, query);
				
				if (answerStrength[results] > 8) {
					Scanner reader = new Scanner(new File("data/filtered/cdc-filtered-"+temp+".txt"));
					
					while (reader.hasNext()) {
						answers[stringCounter][answerCounter] = reader.nextLine();
						answerCounter++;
					}
			...
			for (int i = 0; results > i; i++) {
				if (answerStrength[i] > highestValue) {
					highestValue = answerStrength[i];
					highestPos = i;
				}
			}
			
			for (int i = 0; answers[highestPos].length > i; i++) {
				if (answers[highestPos][i] == null) {
					break;
				}
				
				System.out.println(answers[highestPos][i]);
			}
	...
	/**
	 * Cuts off the email portion of the website
	 * @param input The line being potentially cut
	 * @return A line that does not include the email ending of the CDC website
	 */
	private static String cutEnding(String input) {
		if (input.equals("To receive email updates about this page, enter your email address:") || input.equals("We take your privacy seriously. You can review and change the way we collect information below.")) {
			return "";
		} else {
			return input;
		}
	...
	/**
	 * Breaks up any lines with multiple links
	 * @param input The line with multiple links
	 * @param writer The writer which saves the links
	 * @throws IOException In case an error saving occurs
	 */
